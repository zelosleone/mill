...
object Foo {
  def main(args: Array[String]): Unit = {
    ...
    val result = locationData
      .withColumn("distance_km", 
        expr(s"haversineDistance(latitude, longitude, $refLat, $refLon)"))
      .withColumn("in_service_area",
        expr(s"isInPolygon(latitude, longitude, '${serviceAreaPolygon.mkString(";")}')")
      )
      .withColumn("utm_coords",
        expr("convertToUTM(latitude, longitude)"))
  ...
  val isInPolygon = udf { (lat: Double, lon: Double, polygonStr: String) =>
    ...
    for (i <- polygon.indices) {
      if ((polygon(i)._2 > lon) != (polygon(j)._2 > lon) &&
